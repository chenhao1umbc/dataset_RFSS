...
def create_mixed_signal_analysis(signals, sample_rate, save_path):
    ...
    im = ax.pcolormesh(
        t * 1000, f_mhz, magnitude_db, shading="gouraud", cmap="plasma", rasterized=True
    )
    ...
    plt.suptitle(
        "Multi-Standard RF Signal Source Separation Challenge",
        fontsize=18,
        fontweight="bold",
    )
    plt.tight_layout()
...
def create_constellation_diagrams(signals, save_path):
    ...
    for i, (name, sig) in enumerate(signals.items()):
        ...
        max_val = (
            np.max(
                [np.abs(constellation_points.real), np.abs(constellation_points.imag)]
            )
            * 1.1
        )
...
def main():
    ...
    create_stft_analysis(
        signals, sample_rate, os.path.join(output_dir, "stft_spectrograms.pdf")
    )

    # Create mixed signal analysis
    print("Creating mixed signal analysis...")
    create_mixed_signal_analysis(
        signals, sample_rate, os.path.join(output_dir, "mixed_signal_analysis.pdf")
    )

    # Create constellation diagrams
    print("Creating constellation diagrams...")
    create_constellation_diagrams(
        signals, os.path.join(output_dir, "constellation_diagrams.pdf")
    )
